...
const LogoTicker: React.FC = () => {
  ...
  return (
    <section id="partners" className="py-10 border-y border-gray-100 bg-white/50 backdrop-blur-sm overflow-hidden">
      <div className="relative flex overflow-x-hidden group">
        <div className="animate-scroll flex whitespace-nowrap gap-16 md:gap-32 items-center">
          {[...companies, ...companies, ...companies].map((company, idx) => (
            <div key={idx} className={`text-2xl md:text-3xl text-gray-400 hover:text-brand-dark transition-colors duration-300 select-none ${company.class}`}>
              {company.name}
            </div>
          ))}
        </div>
        ...
        {/* Gradient Masks */}
        <div className="absolute top-0 left-0 h-full w-24 bg-gradient-to-r from-[#FAFAFA] to-transparent z-10"></div>
        <div className="absolute top-0 right-0 h-full w-24 bg-gradient-to-l from-[#FAFAFA] to-transparent z-10"></div>
      </div>
    </section>
  );
};
